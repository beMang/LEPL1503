...
int copy(char *file_name, char *new_file_name)
{
    ...
    if (ftruncate(fd2, size) == -1)
        return error; // Met le fichier à la bonne taille
    ...
    if (msync(new_content, size, MS_SYNC) == -1)
        return error; // On écrit la copie/force la copie

    // On ferme les zones mémoires ainsi que les fichiers :
    if (munmap(content, size) == -1)
        return error;
    if (munmap(new_content, size) == -1)
        return error;
    if (close(fd1) < 0)
        return error;
    if (close(fd2) < 0)
